...
export type Init = {[config: string]: InitConfig};

// InitConfig is a single entry for a configuration step.
...
// InitFile provisions a single file on the target host with the given content, mode, owner, and group.
...
export type InitPackages = string[];

// InitCommand runs a command during initialization with an optional test command for verification afterwards.
export interface InitCommand {
    command: string;
    test?: string;
}

// InitServies is a map from service manager to specific services to manage.
export type InitServices = {[service: string]: InitService};

// InitService is an entry for managing a single service.
